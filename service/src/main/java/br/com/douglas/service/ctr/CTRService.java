...
import br.com.douglas.repository.repositories.ctr.CTRRepository;
    ...
    private void checaSePossuiSaldoSuficiente(CTR ctr) throws DomainException {
        ...
        for (Map.Entry<String, Integer> id : quantidadeDescartePorTipo.entrySet()){
            ...
            if(quantidadeABaixar > saldoDisponivel)
                throw new DomainException(String.format("Quantidade de descarte '%s' excede a quantidade de combo disponível!", ctr.getTipoDescartes().stream().filter(tipoDescarte -> tipoDescarte.getId() == id).findFirst().get().getNome()));
        }
    }
