...
import br.com.douglas.repositories.authority.AuthorityRepository;
...
public class UserService extends BaseService<User> {
    ...
    protected UserService(final UserRepository repository, AuthorityRepository authorityRepository, TokenUtils tokenUtils) {
        super(repository);
    ...
    public void validate(User entity) throws DomainException {
        ...
        Optional<User> userOptionalUsername = repository.findFirstByUsernameLikeIgnoreCase(entity.getUsername());
        if(userOptionalUsername.isPresent())
            throw new DomainException(Message.toLocale("usuario.existente"));
        Optional<User> userOptionalEmail = repository.findFirstByEmailLikeIgnoreCase(entity.getEmail());
        if(userOptionalEmail.isPresent())
            throw new DomainException(Message.toLocale("email.existente"));
    ...
    public User create(User entity) throws DomainException {
        ...
        for (Authority authority : entity.getAuthorities()) {
            auths.add(authorityRepository.findFromName(authority.getName()).orElseThrow(() -> new DomainException(Message.toLocale("authoritie.not-found", authority.getName()))));
        }
        entity.setPassword(tokenUtils.getPasswordEncoder().encode(entity.getPassword()));
